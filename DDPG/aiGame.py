...
# Game主类，支持AI和人类两种模式
class TRexGame:
    def __init__(self, human_mode=False, use_fixed_seed=True):  # Added use_fixed_seed
        pygame.init()
        ...
        self.score = 0
        self.use_fixed_seed = use_fixed_seed  # Store the flag
        if self.use_fixed_seed:
            seed(DEFAULT_SEED)  # Set seed if flag is true
    ...
    def reset(self):
        if self.use_fixed_seed:  # Set seed if flag is true
            seed(DEFAULT_SEED)
